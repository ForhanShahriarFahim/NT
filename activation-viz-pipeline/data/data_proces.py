...
class MakeDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if 'imagenet' in args.data_dir:
            # FIX: Bypass metadata checks and use ImageFolder directly
            val_dir = os.path.join(root, dataset_split) 
            dataset_split = torchvision.datasets.ImageFolder(root=val_dir, transform=transform)
